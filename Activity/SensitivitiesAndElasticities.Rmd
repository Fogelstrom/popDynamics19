...
We will use sensitivity and elasticity analyses to evalutate different management options for the rare and threatened plant mountain golden heather *Hudsonia montana*, which is endemic to North Carolina in USA (from Table 6.7 in Morris & Doak (2002)).  
...
Study the mean matrix and try to understand the life cycle and what the different values mean. This matrix is more complicated than the frog matrix used previously, since plants like mountain golden heather can regress (shrink) in size. Use the mean matrix to project the mountain golden heather population. 


You will have to choose between two management regimes that would focus on  
1)	increasing seed survival in the seed bank, or  
2) increasing seed production in the smallest size class (tiny)  
**what type of management regime should you choose?**

To answer this question, we will perform a sensitivity analysis. The general question for a sensitivity analysis is how a very small change in a particular matrix element affects the population growth rate $\lambda$. You'll find that some matrix elements have a large impact on $\lambda$, whereas other matrix elements do not influence $\lambda$ significantly.  
...
Elasicity values can be interpreted as proportional sensitivities. They measure the relative change in $\lambda$ in response to changes in vital rates. Elasticities are useful because they are comparable (range between 0 and 1) and because impossible transitions get elasticities of zero.
