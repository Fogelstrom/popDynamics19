---
title: 'Exercise 6, Part2: Sensitivities & elasticities: Notes'
output:
  html_document:
    code_folding: hide
...
We will use sensitivity and elasticity analyses to evalutate different management options for the rare herb *Hudosonia montana*, the mountain golden heather. 
...
Matrices with vital rates from four years are included in the `popbio` package. Let's start with installing the package, loading the data and calculating a mean matrix (with mean vital rates) that we will use for the analyses.
...
$$S_{ri} = \frac {\lambda_{1, new}-\lambda_{1, original}} {a_{new}-_{original}}= \frac {\delta\lambda_1} {\delta a_ij}$$
